...
setup_ytdlp(){
    ...
    mkdir -p $HOME/bin
    curl -s -O --output-dir $HOME/bin https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/bin/termux-url-opener
    echo -e "${green}Done.${white}"
}

setup_prettify(){
    echo -e "\n3. Downloading and Setting up Prettify script."
    curl -s -O --output-dir $PATH https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/bin/prettify
...
setup_rxfetch(){
    ...
    curl -s -O --output-dir $PATH https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/bin/rxfetch
...
change_ui(){
    local colors="colors.properties.amoled"
    if ! $defaults; then
        echo -e "\n6. Changing Colour and Font."
        local ui_options="\n1. Set Black Amoled\n2. Set Google Dark"
        declare -A ui_dict
        ui_dict[1]="export colors=colors.properties.amoled"
        ui_dict[2]="export colors=colors.properties.google_dark"
        clear
        ask "${ui_options}" "${green}UI Menu${white}" "ui_dict"
    fi
    curl -s -o $HOME/.termux/colors.properties https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/.termux/"${colors}"
    wget -q -O $HOME/.termux/font.ttf https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/.termux/JetBrainsMonoNL-Regular.ttf
...
save_setup_sh(){
    if ! [ -f $PATH/setup-termux ]; then
        ...
        echo -e '#!/bin/bash\nbash -c "$(curl -fsSL https://raw.githubusercontent.com/niteshraj2310/TermuxStuff/main/setup.sh)"' > $PATH/setup-termux
